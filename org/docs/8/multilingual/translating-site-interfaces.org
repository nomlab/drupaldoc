...
* Translating site interfaces
  この機能が使えるようになると，core の /Interface Translation module/ (Drupal 8 より前のバージョンでは「ロケール」と呼ばれていた機能)」のモジュールを用いると，デフォルト(英語)以外の言語でDrupalのサイトを利用できます．
  ...
  ローカライゼーションアップデートモジュールは http://localize.drupal.org から自動的に翻訳を検索することができます．
  すでに翻訳が完了している言語に対して，あなたはこのモジュールを設定する以外のことをすべきではありません．
  ...
  Drupal翻訳サーバから翻訳ファイルをダウンロードできます．
...
たとえば，'this text' が /some/path に表示されている場合，別の言語に切り替えてから /some/path にアクセスしてください．
  ...
  Drupalで多言語サイトを設定する際に役立つ資料については，「リソースガイド：多言語サイトの設定」を参照してください．

  また，localize.drupal.org の翻訳のセキュリティへの影響についても読んでください．
